...
/****** Object:  UserDefinedFunction [dbo].[XMLGetWordRule]    Script Date: 2020/12/6 10:53:37 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE OR ALTER FUNCTION [dbo].[InnerBMMSplit]
(
...
BEGIN
	...
	--  设置初始值
	SET @SqlResult = '';
	-- PRINT 'InnerBMMSplit>开始分词（“' + @SqlWord + '”）！';
	-- 设置计数器
	SET @SqlIndex = 0;
	SET @SqlCount = LEN(@SqlWord);
	SET @SqlPosition = LEN(@SqlWord) + 1;
	-- 循环处理
	...
	BEGIN
		...
		-- 获得右侧最大内容
		SET @SqlValue = RIGHT(@SqlWord, @SqlCount);
		...
		-- 重置参数
		SET @SqlWord = LEFT(@SqlWord, LEN(@SqlWord) - @SqlCount); SET @SqlCount = LEN(@SqlWord) + 1;
		-- PRINT 'InnerBMMSplit>找到匹配（cid=' + CONVERT(NVARCHAR(MAX), @SqlCid) + '，“' + @SqlValue + '”）！';
	END
	-- 检查结果
	-- 存在有无法识别的内容
	...
	END
	-- 分解成功
	RETURN CONVERT(XML, '<result id="1" met="BMM">' + @SqlResult + '</result>');
END
GO
