...
/****** Object:  UserDefinedFunction [dbo].[BMMFind]    Script Date: 2020/12/6 10:53:37 ******/
...
-- Description:	<使用逆向最大匹配搜索>
-- =============================================

CREATE OR ALTER FUNCTION [dbo].[BMMFind]
(
...
BEGIN
	...
	-- 包括空，单字
	-- 终结符拒绝进一步分割
	IF LEN(@SqlContent) = 1 OR @SqlPosition <= 1
	...
	SET @SqlLeft = 0;
	-- 循环处理
	WHILE @SqlLeft < @SqlPosition - 1
	BEGIN
		-- 设置计数器
		SET @SqlLeft = @SqlLeft + 1;
		-- 获得左侧最大内容
		SET @SqlValue = SUBSTRING(@SqlContent, @SqlLeft, @SqlPosition - @SqlLeft);
	...
	-- 搜索成功
	RETURN CONVERT(XML, '<result id="1" met="BMM" pos="' + CONVERT(NVARCHAR(MAX), @SqlLeft) + '">' + @SqlValue + '</result>');
END
GO
