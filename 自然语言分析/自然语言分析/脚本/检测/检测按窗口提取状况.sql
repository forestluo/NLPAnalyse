...
PRINT '检测按窗口提取状况> 进入正常状态！';

DECLARE @SqlResult XML;
SET @SqlResult = dbo.ExtractAllWords(1, @SqlContent);
...
/*你的SQL脚本结束*/
PRINT '检测按窗口提取状况> [语句执行花费时间(毫秒)] ' + CONVERT(NVARCHAR(MAX),DateDiff(ms, @SqlDate, GetDate()));
