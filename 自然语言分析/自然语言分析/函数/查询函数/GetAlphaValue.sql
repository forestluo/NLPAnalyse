...
-- Description:	<获得两个词的相关系数>
-- =============================================

CREATE OR ALTER FUNCTION GetAlphaValue
(
...
BEGIN
	...
	SET @SqlCombinateCount =
		dbo.GetFreqCount(@SqlLeft + @SqlRight);
	-- 检查加过
	IF @SqlCombinateCount <= 0 RETURN -1;

	-- 计算结果
	SET @SqlValue = @SqlLeftCount;
	SET @SqlValue = @SqlValue * @SqlRightCount;
	SET @SqlValue = @SqlValue / (@SqlLeftCount + @SqlRightCount);
